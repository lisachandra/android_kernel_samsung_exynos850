...
DECLARE_STATE_FUNC(idle)
{
	...
	if (input_booster_event == BOOSTER_ON) {
		int i;
		pr_booster("[Input Booster] %s      State0 : Idle  index : %d, hmp : %d, dma_latency : %d, cpu1 : %d, time : %d, input_booster_event : %d\n", glGage, _this->index, _this->param[_this->index].hmp_boost, _this->param[_this->index].dma_latency, _this->param[_this->index].cpu1_freq, _this->param[_this->index].time, input_booster_event);
		_this->index = 0;
		_this->level = -1;
		for (i = 0; i < 2; i++) {
		...
		pr_booster("[Input Booster] %s      Skipped  index : %d, hmp : %d , dma_latency : %d, cpu1 : %d, input_booster_event : %d\n", glGage, _this->index, _this->param[_this->index].hmp_boost, _this->param[_this->index].dma_latency, _this->param[_this->index].cpu1_freq, input_booster_event);
		pr_booster("\n");
	}
}
